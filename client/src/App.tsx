...
export function App() {
  ...
  useEffect(() => {
    ...
    if (!hasJoined || !roomId) return () => { rendererRef.current = null; };
  ...
  return (
    <main className="shell">
      <header className="topbar">
        ...
        <div className="room-chip"><span className={status === "connected" ? "live-dot" : "live-dot muted"}></span>{status}{hasJoined && <strong>/{roomId}</strong>}</div>
      ...
      {!hasJoined && (
        <section className="intro">
          ...
          <form className="join-form" onSubmit={join}><label>YOUR NAME<input value={name} maxLength={24} onChange={(event) => setName(event.target.value)} /></label><label>ROOM CODE<input value={roomId} maxLength={64} placeholder="Enter a room code" onChange={(event) => setRoomId(event.target.value)} /></label><button type="submit" disabled={!roomId.trim()}>Join room <span>↗</span></button></form>
        </section>
      )}
      {hasJoined && (
